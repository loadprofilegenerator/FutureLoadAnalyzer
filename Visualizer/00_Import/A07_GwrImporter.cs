...
namespace BurgdorfStatistics._00_Import {
    ...
    public class A07_GwrImporter : RunableWithBenchmark {
        ...
        private static void TransferFields([NotNull] [ItemNotNull] object[,] arr, [NotNull] Dictionary<string, int> hdict, int row, [NotNull] GwrData a)
        {
            ...
            //a.DatumderletztenAenderung_GMUTDAT = Helpers.GetDateTime(arr[row, hdict["GMUTDAT"]]);
            //a.DatumdesExports_GEXPDAT = Helpers.GetDateTime(arr[row, hdict["GEXPDAT"]]);
        }

        public A07_GwrImporter([NotNull] ServiceRepository services)
            : base(nameof(A07_GwrImporter), Stage.Raw, 7, services, true)
        {
        }
        ...
        protected override void RunChartMaking()
        {
            ...
            var gwr = dbRaw.Database.Fetch<GwrData>();
            Log(MessageType.Info, "Loaded gwr data: " + gwr.Count);
            var ebd = dbRaw.Database.Fetch<EnergiebedarfsdatenBern>();
            Log(MessageType.Info, "Loaded energiebedarfsdaten " + ebd.Count);
            MakeMap();
            MakeGWRArea();
            MakeAnzahlGeschosse();
            MakeAnzahlWohnungen();
            Log(MessageType.Info, "Testmap written");

            void MakeAnzahlWohnungen()
            {
                const string sectionDescription = "Gesammelte GWR Visualisierungen";
                ...
                MakeOneAnalysis(whg, bucketSize, baseName,  sectionDescription, slice);
            }
            ...
            void MakeGWRArea()
            {
                //const string section = "GWR - GArea";
                const string sectionDescription = "Gesammelte GWR Visualisierungen";
                ...
                MakeOneAnalysis(areas, bucketSize, baseName,   sectionDescription,slice);
            }
            void MakeMap()
            {
                ...
                foreach (var data in gwr) {
                    ...
                    if (ebdo != null) {
                        ...
                        Log(MessageType.Warning, "No area entry from energiebedarfsdaten #" + missingEntries++, data);
                    }
                ...
                const string sectionDescription = "Zeit die Verteilung der Wohnungen";
                string fullName = MakeAndRegisterFullFilename("HousesAndApartmentCounts.svg", Name, sectionDescription, Constants.PresentSlice);
                Services.PlotMaker.MakeMapDrawer(fullName, "", points, new List<MapLegendEntry>(), MyStage);
            }
        }

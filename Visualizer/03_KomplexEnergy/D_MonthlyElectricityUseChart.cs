...
namespace BurgdorfStatistics._03_KomplexEnergy {
    ...
    public class D_MonthlyElectricityUseChart : RunableWithBenchmark {
        ...
        {
            var db = SqlConnection.GetDatabaseConnection(Stage.ComplexEnergyData, Constants.PresentSlice);
            var energyusePerStandort = db.Database.Fetch<MonthlyElectricityUsePerStandort>();
            ...
            MakeSortedChart(energyusePerStandort, db);
            foreach (var vba in vbas) {
                MakeChartForOneVerbrauchsart(vba, 1000, "MWh", energyusePerStandort, db);
            }
        }

        private void MakeSortedChart([NotNull] [ItemNotNull] List<MonthlyElectricityUsePerStandort> energyusePerStandort, [NotNull] MyDb mydb)
        {
            var path = mydb.GetResultFullPath(SequenceNumber, Name);
...

            {
                ...
                var idx = 0;
                double sum = 0;
                var electricityPaes = new List<PlotMaker.AnnotationEntry>();
                foreach (var standort in electricityEntriesWithValues) {
                    ...
                    foreach (var plannedAnnotationse in pas) {
                        if (standort.YearlyElectricityUseLocalnet < plannedAnnotationse.Limit && !plannedAnnotationse.Processed) {
                            electricityPaes.Add(new PlotMaker.AnnotationEntry(plannedAnnotationse.MakeDescription(electricityEntriesWithValues), idx, sum, plannedAnnotationse.Direction,
                ...
                electricityPaes.Add(new PlotMaker.AnnotationEntry(zeroAnno.MakeDescription(electricityEntriesWithValues), idx, sum, 1, 50));
                bses.Add(lse);
                Services.PlotMaker.MakeLineChart(Path.Combine(path, "SortedCustomers.Electricity.png"), "Strom [kWh]", bses, electricityPaes);
            }
            {
                var pas = new List<PlannedAnnotations>();
                var zeroAnno = new PlannedAnnotations(0, 500, 1, 100);
                ...
                var bses = new List<LineSeriesEntry>();
                var lse = new LineSeriesEntry("Electricity");
                var gasPaes = new List<PlotMaker.AnnotationEntry>();
                var idx = 0;
                double sum = 0;
                foreach (var standort in gasEntriesWithValues) {
                    ...
                    foreach (var plannedAnnotationse in pas) {
                        if (standort.YearlyGasUse < plannedAnnotationse.Limit && !plannedAnnotationse.Processed) {
                            gasPaes.Add(new PlotMaker.AnnotationEntry(plannedAnnotationse.MakeDescription(gasEntriesWithValues), idx, sum, plannedAnnotationse.Direction, plannedAnnotationse.YOffset));
                            plannedAnnotationse.Processed = true;
                        }
                    }
                ...
                gasPaes.Add(new PlotMaker.AnnotationEntry(zeroAnno.MakeDescription(gasEntriesWithValues), idx, sum, 1, 50));
                bses.Add(lse);
                Services.PlotMaker.MakeLineChart(Path.Combine(path, "SortedCustomers.Gas.png"), "Strom [kWh]", bses, gasPaes);
            }
        }
        ...
        private class PlannedAnnotations {
            ...
            {
                var l = monthlyElectricityUsePerStandorts.Where(x => x.YearlyElectricityUseLocalnet >= Limit && x.YearlyElectricityUseLocalnet < NextLimit).ToList();
                return "Anzahl Kunden\nzwischen " + Limit + " und " + NextLimit + " kWh:\n" + l.Count;
            }
        }

        private void MakeChartForOneVerbrauchsart(Verbrauchsart vba, double factor, [NotNull] string unit, [NotNull] [ItemNotNull] List<MonthlyElectricityUsePerStandort> energyusePerStandort,
                                                  [NotNull] MyDb db)
        {
            ...
            foreach (var tarifValues in energyUseByTarifAndMonth) {
                ...
                Services.PlotMaker.MakeBarChart(Path.Combine(path, "MonthlyUse." + vba + "." + Helpers.CleanFilename(bse.Name) + ".png"), "Strom [kWh]", singleTarif, new List<string>());
            }
        ...
        [NotNull]
        private static Dictionary<string, Dictionary<int, double>> GetDictForVerbrauch([NotNull] MonthlyElectricityUsePerStandort perStandortElectricityMonthlyValuesByTarif, Verbrauchsart vba)
        {
            switch (vba) {
        ...
        public D_MonthlyElectricityUseChart([NotNull] ServiceRepository services)
            : base(nameof(D_MonthlyElectricityUseChart), Stage.ComplexEnergyData, 4, services, true)
        {
        }
    }
}
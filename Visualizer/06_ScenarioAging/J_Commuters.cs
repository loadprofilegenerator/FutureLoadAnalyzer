...
namespace BurgdorfStatistics._06_ScenarioAging {
    ...
    public class J_Commuters : RunableForSingleSliceWithBenchmark {
        public J_Commuters([NotNull] ServiceRepository services)
            : base(nameof(J_Commuters), Stage.ScenarioCreation, 1000, services, false)
        {
        ...
        protected override void RunActualProcess([NotNull] ScenarioSliceParameters parameters)
        {
            ...
            SqlConnection.RecreateTable<HeatingSystemEntry>(Stage.Houses, up.DstScenario, up.DstYear);
            var dbSrcHouses = SqlConnection.GetDatabaseConnection(Stage.Houses, up.SrcScenario, up.SrcYear).Database;
            var dbDstHouses = SqlConnection.GetDatabaseConnection(Stage.Houses, up.DstScenario, up.DstYear).Database;
